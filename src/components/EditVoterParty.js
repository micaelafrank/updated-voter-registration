import React, { useState } from "react";
import Row from "react-bootstrap/Row";
import Col from "react-bootstrap/Col";
import { FaEdit, FaRegCheckCircle } from "react-icons/fa";


function EditVoterParty({ party, id }) {
    const [partyState, setPartyState] = useState("");
    const [editState, setEditState] = useState(false);
    const [initialPartyValue, setInitialPartyValue] = useState(party);


    function handleEdit() {
        setEditState(!editState);
        if (partyState !== initialPartyValue) {
            fetch(`/editvoter/${id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({
                    party: partyState,
                    id: id,
                }),
            })
                .then((resp) => resp.json())
                .then((data) => setInitialPartyValue(data.party));
        }
    };


    return (
        <>
            {editState ?
                (<div>
                    {/* <p id="fullNameTitle" style={{ fontSize: "18px", textAlign: "center", alignItems: "center", fontWeight: "bold", lineHeight: ".4" }}>{initialFNValue}</p> */}
                    <Row className="mt-2" style={{ display: "flex", flexDirection: "row" }}>
                        <Col>
                            <select
                                style={{ fontSize: "14px", fontFamily: "monospace", padding:"5px" }}
                                onChange={(e) => setPartyState(e.target.value)}
                                defaultValue={initialPartyValue}
                            >
                                <option>Democrat</option>
                                <option>Green</option>
                                <option>Independent</option>
                                <option>Republican</option>
                                <option>Working Families</option>
                            </select>
                            {/* <textarea
                                className="form-controlFN"
                                defaultValue={initialFNValue}
                                onChange={(e) => setFirstNameState(e.target.value)}
                            /> */}
                        </Col>
                        {editState ? <FaRegCheckCircle id="checkButton" onClick={handleEdit} /> : <FaEdit id="editButton" onClick={handleEdit} />}
                    </Row>
                </div>
                ) : (
                    // </InputGroup>
                    // </ListGroup.Item>
                    // <ListGroup.Item className="text-center">
                    <Row className="my-2 text-center" style={{ display: "flex", flexDirection: "row" }}>
                        <Col className="px-3">
                            <span>{initialPartyValue}</span>                   
                        </Col>
                        {editState ? <FaRegCheckCircle id="checkButton" onClick={handleEdit} /> : <FaEdit id="editButton" onClick={handleEdit} />}
                    </Row>
                )
            }

            {/* {editState ? <FaRegCheckCircle id="checkButton" onClick={handleEdit} /> : <FaEdit id="editButton" onClick={handleEdit} />} */}
            {/* </Button> */}
        </>
    )
}

export default EditVoterParty;